...
export async function POST(request) {
  try {
    ...
    if (!ALLOWED_MIME_TYPES.has(file.type)) {
      return Response.json({ error: 'Only image files are allowed (jpg, png, webp, gif, svg)' }, { status: 400 });
    }

    // Validate file extension — MIME type alone can be spoofed by renaming a file
    const ext = path.extname(file.name).toLowerCase();
    if (!ALLOWED_EXTENSIONS.has(ext)) {
      return Response.json({ error: 'Invalid file extension. Allowed: jpg, jpeg, png, webp, gif, svg' }, { status: 400 });
    }
    ...
    try {
      ...
      if (token) {
        ...
        await logActivity(payload.email, 'UPLOAD_MEDIA', { filename: file.name, path: publicPath });
      }
    } catch (e) {}
  ...
  try {
    ...
    try {
      ...
      await prisma.mediaAsset.deleteMany({
        where: { url: filePath } // assuming filePath matches the saved url
      });
      await prisma.$disconnect();
    } catch (dbErr) {
